	...
	if ( ( 1 != $set_flags ) || ( 1 == $set_flags ) )  {
			...
			foreach ($social_links as $key => $value) {
				if (!empty($options[$value])) {
					$a .=
					'<li class="'.strtolower($key).'"><a href="'.esc_url($options[$value]).'" title="'.sprintf(esc_attr__('%1$s on %2$s', 'ultimate'), get_bloginfo('name'), $social_links_name[$i]).'" target="_blank" rel="noopener noreferrer">'.'</a></li>';
				}
				$i++;
			}
			if (($i > 0) && (!empty($a))) {
